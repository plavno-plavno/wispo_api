...
%%start() ->
%%    Config = rbac_config:get(rbac, ?MODULE),
%%    start(Config).
%%
%%-spec start(proplists:proplist()) -> {ok, pid()} | {error, term()}.
%%start(Args) ->
%%    gen_server:start({local, ?MODULE}, Args, ?GEN_SERVER_OPTS).
...
%% Extended Query
%%
%% {ok, C} = rbac_db_pg_worker:start().
%% gen_server:call(C, {squery, "SELECT * FROM rbac_objects WHERE name = '*'"}).
%% rbac_db_pg_worker:squery(<<"SELECT * FROM rbac_objects WHERE name = '*'">>).
...
%% https://github.com/epgsql/pgapp/blob/e55452ba8621720bd9bfe0b5d0cd92687c82d470/src/pgapp.erl#L84
%% https://github.com/epgsql/pgapp/blob/e55452ba8621720bd9bfe0b5d0cd92687c82d470/src/pgapp_worker.erl#L70
...
%% rr(code:lib_dir(epgsql) ++ "/include/epgsql.hrl").
%% rbac_db_pg_worker:start().
%% rbac_db_pg_worker:connect().
%% Reply1 = rbac_db_pg_worker:squery("SELECT * FROM rbac_users WHERE 1=1").
%% Reply2 = rbac_db_pg_worker:squery("SELECT * FROM test_table WHERE id = 1").
%% rbac_db_pg_worker:normalize_epgsql_reply(Reply1).
%% rbac_db_pg_worker:normalize_epgsql_reply(Reply2).
...
% TODO: Compare SQL Query and Reply (count of inserted/updated/deleted records)?
